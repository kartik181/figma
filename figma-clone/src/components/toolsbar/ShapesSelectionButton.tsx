...
export default function ShapesSelectionButton({
  ...
  onClick: (
    layerType: LayerType.Rectangle | LayerType.Ellipse | LayerType.Triangle,
  ) => void;
}) {
    ...
    <div className="relative flex" ref={menuRef}>
      ...
      >
        ...
        {canvasState.mode === CanvasMode.Inserting &&
          canvasState.layerType === LayerType.Triangle && (
            <IoTriangleOutline className="h-5 w-5" />
          )}
      </IconButton>
      ...
      {isOpen && (
        <div className="absolute -top-20 mt-1 min-w-[150px] rounded-xl bg-[#1e1e1e] p-2 shadow-lg">
          ...
          <button
            className={`flex w-full items-center rounded-md p-1 text-white hover:bg-blue-500 ${canvasState.mode === CanvasMode.Inserting && canvasState.layerType === LayerType.Triangle ? "bg-blue-500" : ""}`}
            onClick={() => handleClick(LayerType.Triangle)}
          >
            <span className="w-5 text-xs">
              {canvasState.mode === CanvasMode.Inserting &&
                canvasState.layerType === LayerType.Triangle &&
            ...
            <IoTriangleOutline className="mr-2 h-4 w-4" />
